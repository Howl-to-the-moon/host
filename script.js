...
let rainBgm = new Audio(window.isThunder ? "https://dimden.dev/sounds/rainHeavy3.mp3" : "https://dimden.dev/sounds/rainLight1.mp3");
let thunderBgm;
let updateRain;
let isWinter = new Date().getMonth() == 10 || new Date().getMonth() == 11 || new Date().getMonth() == 0 || new Date().getMonth() == 1;
window.isSnowSeason = isWinter;
let snowBgm = new Audio("https://dimden.dev/sounds/snow.ogg");
...
setTimeout(() => {
    ...
    let maintd = document.getElementById("maintd");
    let maintdRect = maintd && maintd.getBoundingClientRect() || { top: 0, left: 0, right: 0, bottom: 0 };
    ...
    document.addEventListener("mousemove", e => {
        ...
        if(maintd && y > maintdRect.top && x > maintdRect.left && x < maintdRect.right) {
            rainBgm.volume = 0.05;
        } else {
            rainBgm.volume = 0.2;
        }
    ...
    function thunder() {
        if(window.isTabFocused) playThunder();
    ...
    updateRain = function() {
        ...
        const now = Date.now();
        const adjustedSpeed = Math.round((speed * 60) / window.cfps || 30);
